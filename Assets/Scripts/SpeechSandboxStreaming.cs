...
public class SpeechSandboxStreaming : MonoBehaviour
{
    ...
    //este metodo es el encargado de hacer la traducción  speech-to.
    private void OnRecognize(SpeechRecognitionEvent result, Dictionary<string, object> customData = null)
    {
        if (result != null && result.results.Length > 0)
        {
            // result tiene results posibles -> res y estos a su vez alternativas -> alt
            foreach (var res in result.results)
            {
                foreach (var alt in res.alternatives)
                {
                    if (res.final && alt.confidence > 0) // la confianza de la posible palabra (alternativa) debe ser mayor a cero
                    {
                        string text = alt.transcript;
                        Debug.Log("Result: " + text + " Confidence: " + alt.confidence);

                        //se le pasa el texto y se llama la función OnMessage, que se encarga de parsear y determina como manejar los comandos de voz
    ...
    // Se encarga de parsear y determina como manejar los comandos de voz
    void OnMessage(object resp, Dictionary<string, object> customData)
    {
        ...
        //toma la info de resp y la mete en fsdata
        ...
        //toma la info de fsdata y la mete en obj
        r = _serializer.TryDeserialize(fsdata, obj.GetType(), ref obj);
        if (!r.Succeeded)
            throw new WatsonException(r.FormattedMessages);

        //LOS POSIBLES INTENTS Y ENTITIES USADAS, OBTENIDAS DEL MESSAGERESPONSE, ESTÁN DEFINIDAS EN MI ARCHIVO data/workspace.json,
        //DICHO ARCHIVO FUE CARGADO EN EL RECUSRO ASSISTENT DE MI CUENTA DE IBM C

        //si llega un mensaje reconocido y contiene intents (acciones sobre los objetos) o entidades (propiedades sobre los objetos)
        if (resp != null && (messageResponse.intents.Length > 0 || messageResponse.entities.Length > 0))
        {
            ...
            //Si el intent es de una acción move
            if (intent == "move")
            {
                //las entities en el mensaje deben ser orientadas a la dirección en la que se quiere mover el objeto
                foreach (RuntimeEntity entity in messageResponse.entities)
                {
                    Debug.Log("entityType: " + entity.entity + " , value: " + entity.value);
                    //por tanto el valor de cada entity se le asigna a la variable dirección del OJ (objeto de juego) actual
                    direction = entity.value;
                    //y se aplica dicha dirección al OJ
